...
- 🔴 quando casar gatilho vermelho (cite o gatilho e a base legal em Observações/Irregularidades
  e a ação da tabela em Ação Sugerida);
...
[
  {
    ...
    "base_legal": "Art. 125, Lei 14.133/21",
    "nivel_atencao": "🔴",
    "observacoes": "3º termo aditivo ao mesmo contrato — gatilho automático",
    "acao_sugerida": "Requerimento",
    "pagina": 24
  }
]
```
...
é sempre um de 🔴/🟡/🟢; `base_legal`/`acao_sugerida` vazios quando 🟢 sem gatilho; `pagina` é o
número impresso (o do `## Página N`). Não inclua atos fora das categorias-alvo. Não inclua campo
`status` (o publicador grava "Novo").
